...
export class WidgetSettingsComponent implements OnInit {
  ...
  private initializeSettings(): void {
    this.chartSettings = {
      chartType: this.widget.chartType || 'line',
...
private buildChartConfig(): any {
  const type = this.chartSettings.chartType;
  const isDoughnut = type === 'doughnut';
  const chartTypeForHighcharts = isDoughnut ? 'pie' : type;

  return {
    chart: {
      type: chartTypeForHighcharts,
      height: 250,
    },
    title: {
      text: this.chartSettings.title || (isDoughnut ? 'Doughnut Chart' : `${type.charAt(0).toUpperCase() + type.slice(1)} Chart`),
      style: { fontSize: '16px', fontWeight: 'bold' }
    },
    legend: {
      enabled: this.chartSettings.showLegend,
      align: 'bottom',
    ...
    plotOptions: {
      [chartTypeForHighcharts]: {
        innerSize: isDoughnut ? '60%' : undefined,
        dataLabels: {
          enabled: this.chartSettings.showDataLabels,
          format: chartTypeForHighcharts === 'pie'
